...
public class LatticeGameTests {
    ...
    public static void codexHasValidItemsAndSpoilerSafeLanding(GameTestHelper h) {
        ...
        h.assertTrue(entries.stream().anyMatch(e->"Carry the beat".equals(e.title()) && e.text().contains("fill the cell from that store") && e.text().contains("25 Pulse per use") && e.text().contains("voice-craft") && e.text().contains("Ley Collector does not fill")),"A cell fills from a Drumheart (25), Resonator or voice-craft (100); a Ley Collector does not fill a cell");
