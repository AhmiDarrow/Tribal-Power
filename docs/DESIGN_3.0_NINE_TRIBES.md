...
- **Echo Unweave** station (`echo_unweave`, 5th Echo station, Loom attunement): reverses the lattice — Manifested Ingot → 2 Bound Echo, Bound Echo → 2 Attuned Echo, Attuned Echo → 2 Echo Shards, Spiritweave → 2 Wool, Resonant Core → 3 Manifested Ingots; plus tool salvage: any damaged Spiritgear tool → 1 Manifested Ingot (`tribalpower:damaged` ingredient type wrapping `#tribalpower:spiritgear_tools`; undamaged tools do not match). Datapack type `tribalpower:lattice` with `"station": "echo_unweave"`.
...
- **Codex gating** (§2, §3): tribe crest pages unlock when the Elder hands over the Tribe Mark (Voice rank, `TribeStandingSavedData` marks bitmask) and Lore Tablet pages when that tablet is read (`PlayerPersisted.TribalTabletsRead`); the server sends both bitmasks in `tribalpower:codex_unlocks` on login, respawn/clone, Mark grant and first read. The session-wide spoiler toggle still reveals everything (creative/single-player override, showcase tour). Meeting a tribe (`tribalpower_tribes_met`) only drives the greeting.
